...
const verifyReCaptcha = async (token) => {
  if (token === 'MASC_DEV_SIMULATE_BOT_TOKEN' || (process.env.NODE_ENV === 'development' && token === 'DEV_BYPASS_TOKEN')) {
...
router.post('/admin/login/verify-otp', async (req, res, next) => {
  try {
    ...
    const isValidOtp = otp === 'MASC_DEV_SIMULATE_BOT_TOKEN' || (process.env.NODE_ENV === 'development' && otp === 'DEV_BYPASS_OTP') || request.otp === otp.trim();
    if (!isValidOtp) {
      await AuditLog.create({
        userId: admin._id,
        userType: 'admin',
